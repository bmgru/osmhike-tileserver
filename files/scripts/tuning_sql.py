...
#-------------------------------------------------------------------------
# print a text , with the adequate number of =  under it
#-------------------------------------------------------------------------
def niceprint(txt):
    print("\n" + txt)
    # print as many = than the length of txt
    print(''.join('=' for _ in txt )) 
    print("\n")
...
# go through all layers, extract the sql request , execute it
# if flag --explain is set, reexecute "EXPLAIN sqlrequest" to investigate performane
for l in yml['Layer']:
    if (
        ...
        if (
            ...
            layers += 1
            # initial sql request associated with the layer
            sql = l['Datasource']['table']
            # process magic tokens
            ...
            " final request as it would be executed by Mapnik"
            sql = "SELECT data.*, octet_length(data.*::text) FROM (SELECT ST_nPoints(way), * FROM " +  "\n" + sql + " where way && " + bbox + ") AS data"

            niceprint( 'Layer %s:' % l['id']  )
